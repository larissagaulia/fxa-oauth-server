...
// TODO: better names and only active tokens, not all.
const QUERY_ACTIVE_SERVICES_FOR_EMAIL = 'SELECT c.name, c.id, t.scope FROM fxa_oauth.clients AS c, fxa_oauth.tokens as t WHERE c.id = t.clientId AND email = ?';
const QUERY_DELETE_TOKENS_BY_SERVICE = 'DELETE FROM tokens WHERE clientId=?';
...
MysqlStore.prototype = {
   ...
   * @returns {*}
   */
  getActiveServicesByEmail: function getActiveServicesByEmail(email) {
    if (! email) {
      return P.reject(new Error('Email is required'));
    }

    return this._read(QUERY_ACTIVE_SERVICES_FOR_EMAIL, [
      email
    ]);
  },

  deleteTokensByService: function deleteTokensByService(serviceId) {
    if (! serviceId) {
      return P.reject(new Error('serviceId is required'));
    }

    return this._write(QUERY_DELETE_TOKENS_BY_SERVICE, [
      buf(serviceId)
    ]);
  },
